//getSource() 和 setSource(code) 在initpage.js 中定义 用于获取编辑器代码或者设置编辑器代码
...
function Compiler(domRoot, windowLocalPrefix, lang) {
    ...
    // // var codeText = getSource();

    // //如果code为空 ，，获得默认值
    // if (!codeText) {
    // 	setSource('//type your code here!');
    ...
    function serialiseState() {
        var state = {
            sourcez: LZString.compressToBase64(getSource()),
            compiler: $('.compilerSelect').val(),
            language: $('.languageSelect').val(),
            options: ""
        };
    ...
    function onCompilerChange() {
        ...
        onChange();
        var compiler = compilersById[compiler];
    ...
    function setCompilers(compilers, defaultCompiler,language) {
        ...
        console.log("sdf"+defaultCompiler);
