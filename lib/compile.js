...
temp.track();
var GCC_runningTimeout = props.get("IDE-for-oj","GCC_runningTimeoutMs",10000);
var java_runningTimeout = props.get("IDE-for-oj","java_runningTimeoutMs",10000);
...
//编译动作
Compile.prototype.compile = function (source, compiler, options, inputData, language, requireCompile) {
    var self = this;
                ...
                return new Promise(function (resolve) {
                    if (!requireCompile) {
                        ...
                        if(language == "java"){
                            ...
                            console.log("命令：" + cmd);
                            child_process.exec(cmd,{timeout:GCC_runningTimeout},
                            function (err, stdout, stderr) {
                                if(err){
                                    result.stderr +="\nrunning timeout！";
                                    resolve(result);
...
function compileHandler(compilers) {
    ...
    return function compile(req, res, next) {
        ...
        var requireCompile = req.body.requireCompile;
        var options = req.body.options.split(' ').filter(function (x) {
            return x !== "";
        });
        compileObj.compile(source, compiler, options, inputData, language, requireCompile).then(
            //返回结果信息
            function (result) {
...
var test = 0;
//压力测试编译请求处理主函数
function compileTest(compilers) {
    var compileObj = new Compile(compilers);
    return function compile(req, res, next) {
    	test++;
        var compiler = "/usr/bin/g++";
        var language = "C++";
        console.log("compiler"+compiler+" : language:"+language +test);
        var source = '#include <iostream>\nusing namespace std;\nint main(void){cout<<"hello world!'+test.toString()+'"<<endl;\nreturn 0;}';
        var inputData = "";
        if (typeof("") !== "string") {
            return next(new Error("Bad request"));
        }
        var requireCompile = true;
        var options = "".split(' ').filter(function (x) {
            return x !== "";
        });
        compileObj.compile(source, compiler, options, inputData, language, requireCompile).then(
            //返回结果信息
